-- Migration 089: Tournament podium lists (gold / silver / bronze) in get_player_stats
...
BEGIN
  ...
  -- Tournament podium: one row per tournament per placement tier
  WITH tm AS (
    SELECT
      m.tournament_id,
      t.title,
  ...
  SELECT
    COALESCE(
      (SELECT jsonb_agg(row_to_json(g)::jsonb ORDER BY g.start_at DESC)
       FROM (
         SELECT tournament_id, title, start_at
         FROM deduped WHERE placement = 'gold'
       ) g),
      '[]'::jsonb
    ),
    COALESCE(
      (SELECT jsonb_agg(row_to_json(s)::jsonb ORDER BY s.start_at DESC)
       FROM (
         SELECT tournament_id, title, start_at
         FROM deduped WHERE placement = 'silver'
       ) s),
      '[]'::jsonb
    ),
    COALESCE(
      (SELECT jsonb_agg(row_to_json(b)::jsonb ORDER BY b.start_at DESC)
       FROM (
         SELECT tournament_id, title, start_at
         FROM deduped WHERE placement = 'bronze'
       ) b),
      '[]'::jsonb
    )
  ...
  RETURN jsonb_build_object(
    ...
    'tournament_podium', jsonb_build_object(
