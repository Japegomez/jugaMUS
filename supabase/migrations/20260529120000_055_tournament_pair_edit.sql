-- 055: Edit and remove tournament pairs before bracket generation (organizer only).

CREATE OR REPLACE FUNCTION public.remove_tournament_pair(p_pair_id UUID)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  DELETE FROM public.tournament_pairs WHERE id = p_pair_id;
END;
$$;
...
REVOKE ALL ON FUNCTION public.remove_tournament_pair(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.remove_tournament_pair(UUID) TO authenticated;
