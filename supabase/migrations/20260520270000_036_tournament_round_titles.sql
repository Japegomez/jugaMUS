-- 036: Use proper round names (Cuartos, Semifinal, …) for all tournament match titles.

CREATE OR REPLACE FUNCTION public.tournament_round_name(p_round_size INT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE p_round_size
    WHEN 2 THEN 'Final'
    WHEN 4 THEN 'Semifinal'
    WHEN 8 THEN 'Cuartos'
    WHEN 16 THEN 'Octavos'
    WHEN 32 THEN 'Dieciseisavos'
    ELSE 'Ronda'
  END;
$$;

CREATE OR REPLACE FUNCTION public.tournament_match_title(
  p_tournament_title TEXT,
  p_round_size INT,
  p_is_bye BOOLEAN DEFAULT FALSE
)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT p_tournament_title || ' — ' || public.tournament_round_name(p_round_size)
    || CASE WHEN COALESCE(p_is_bye, FALSE) THEN ' (bye)' ELSE '' END;
$$;

-- Backfill existing tournament matches.
UPDATE public.matches m
SET
  title = public.tournament_match_title(t.title, m.tournament_round_size, m.tournament_is_bye),
  updated_at = NOW()
FROM public.tournaments t
WHERE m.tournament_id = t.id
  AND m.tournament_round_size IS NOT NULL;
...
BEGIN
  ...
  IF v_next_match.tournament_pair_a_id IS NOT NULL
    ...
    v_round_title := public.tournament_match_title(v_tournament.title, v_next_round_size, FALSE);
