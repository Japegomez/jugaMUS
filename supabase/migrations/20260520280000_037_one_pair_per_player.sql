-- 037: One registered player per tournament pair at a time.

CREATE OR REPLACE FUNCTION public.user_is_in_tournament_pair(
  p_tournament_id UUID,
  p_user_id UUID,
  p_exclude_pair_id UUID DEFAULT NULL
)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
AS $$
  SELECT EXISTS (
    ...
    WHERE tp.tournament_id = p_tournament_id
      AND (p_exclude_pair_id IS NULL OR tp.id <> p_exclude_pair_id)
      AND (tp.player_a_user_id = p_user_id OR tp.player_b_user_id = p_user_id)
  );
$$;
