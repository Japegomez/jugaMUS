-- 032: Propagate bye winners into the next bracket round.

CREATE OR REPLACE FUNCTION public.propagate_tournament_winners(p_tournament_id UUID)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_match_id UUID;
BEGIN
  IF p_tournament_id IS NULL THEN RETURN; END IF;

  -- Multiple passes cover bye chains that skip several rounds at bracket generation.
  FOR v_pass IN 1..6 LOOP
    FOR v_match_id IN
      SELECT m.id
      FROM public.matches m
      WHERE m.tournament_id = p_tournament_id
        AND m.tournament_winner_pair_id IS NOT NULL
      ORDER BY m.tournament_round_size DESC, m.tournament_bracket_position ASC
    LOOP
      PERFORM public.advance_tournament_round(v_match_id);
    END LOOP;
  END LOOP;
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.propagate_tournament_winners(UUID) TO authenticated;
...
BEGIN
  FOR v_tid IN
    SELECT id FROM public.tournaments WHERE bracket_generated_at IS NOT NULL
  LOOP
