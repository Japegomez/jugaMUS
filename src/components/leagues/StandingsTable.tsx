...
const STAT_COLUMNS = ['PJ', 'PG', 'PP', 'PTS'] as const
      ...
      {rows.map((row) => {
        ...
        return (
          <View key={row.pair_id} style={[styles.row, isPodium && styles.rowPodium]}>
            <Text style={[styles.cell, styles.rankCell, styles.rankText, isPodium && styles.rankPodium]}>
              {row.rank}
            </Text>
            ...
            <Text style={[styles.cell, styles.statCell, styles.points]}>{row.wins}</Text>
          </View>
        )
      })}
    </View>
...
const styles = StyleSheet.create({
  ...
  points: { color: Colors.textPrimary, fontFamily: Fonts.bold },
})
